...
## Current branch note: c2c_dma_jrf_128_to_64
This branch captures the 64-bit AXI / 8 Gb/s C2C bandwidth-test configuration used for the best observed tarball build. The main EMP Rev2 test targets use 100 MHz AXI infrastructure, 64-bit full AXI C2C links, 32-bit AXI-Lite C2CB links, and CPLL-based 8 Gb/s Aurora links.

The principal branch-specific test targets are:
 - EMP_Cornell_rev2_p1_VU13p-1-SM_USP
 - EMP_Cornell_rev2_p2_VU13p-1-SM_USP

The default target script also includes the p1/p2 `VU13p-2` EMP variants and Rev3 EMP variants for compatibility checks.
...
For this branch's C2C bandwidth-test targets, typical builds are:

  `make EMP_Cornell_rev2_p1_VU13p-1-SM_USP`

  `make EMP_Cornell_rev2_p2_VU13p-1-SM_USP`

The helper script `build_targets.sh` runs prebuild and address-table generation for its default EMP target list, or for a comma-separated `CM_FPGA_FW_TARGET_LIST` override.
...
### Organization:
  ...
  Branch-specific C2C settings live under `src/CM_yaml/CM_C2C`. The Rev2 EMP bandwidth-test configs include the `_8g` C2C YAML variants, which set the full AXI C2C link to 64-bit width and the C2CB AXI-Lite link to 32-bit width.
